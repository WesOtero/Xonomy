...
public class SpecimenController {
	...
	public ResponseEntity<Specimen> addSpecimen(@RequestBody Specimen specimen) {
		Specimen newSpecimen = specimenService.addSpecimen(specimen);
		return new ResponseEntity<Specimen>(newSpecimen, HttpStatus.CREATED);
	}
	
	@GetMapping("/all")
	public ResponseEntity<List<Specimen>> getAllSpecimens() {
		List<Specimen> specimenList = specimenService.getSpecimenList();
		return new ResponseEntity<List<Specimen>>(specimenList, HttpStatus.OK);
	}
	
	@GetMapping("/find/{id}")
	public ResponseEntity<Specimen> getSpecimenById(@PathVariable Long id) {
		Specimen specimen = specimenService.getSpecimenById(id);
		return new ResponseEntity<Specimen>(specimen, HttpStatus.OK);
	}
	
	@PutMapping("/update")
	public ResponseEntity<Specimen> updateSpecimen(@RequestBody Specimen specimen) {
		Specimen updatedSpecimen = specimenService.updateSpecimen(specimen);
		return new ResponseEntity<Specimen>(updatedSpecimen, HttpStatus.OK);
	}
